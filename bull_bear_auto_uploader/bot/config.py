...
def load_config(path: str = "config.yaml") -> dict:
    ...
    cfg["post_every_hours"] = max(
        1,
        int(os.getenv("POST_EVERY_HOURS", "3"))
    )

    cfg["run_on_start"] = env_bool("RUN_ON_START", True)

    cfg["privacy"] = os.getenv(
        "YOUTUBE_PRIVACY",
        "public"
    ).strip().lower()

    cfg["made_for_kids"] = env_bool(
        "MADE_FOR_KIDS",
        False
    )

    cfg["contains_synthetic_media"] = env_bool(
