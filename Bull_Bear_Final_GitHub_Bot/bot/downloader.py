...
def _opts(cfg, download=False):
    opts = {"quiet": True, "no_warnings": True, "ignoreerrors": True, "noplaylist": False, "socket_timeout": 25}
...
def _is_pin(url): return "/pin/" in url.lower()
...
def _extract_pin_ids(page):
    ...
    for pattern in PIN_PATTERNS:
        ids.extend(m.group(1) for m in pattern.finditer(page))
    return ids
...
def _get_profile_pin_urls(profile_url, cfg):
    ...
    pages = [base]
    # Pinterest can serve different embedded pin sets by locale/domain and UA.
    ...
    for url in pages:
        for mobile in (False, True):
            try:
                page = _fetch_profile_html(url, mobile=mobile)
            except Exception as e:
                print("Profile page fetch failed:", url, e); continue
            found = _extract_pin_ids(page)
            print("Raw pin IDs found:", len(found), "from", url, "mobile=" + str(mobile))
            for pin_id in found:
                if pin_id not in seen:
                    seen.add(pin_id); pin_ids.append(pin_id)

    # Also let yt-dlp inspect the profile/board itself; on some Pinterest layouts this
    # exposes entries that are absent from initial HTML.
    try:
        ...
        for entry in entries:
            ...
            eurl = str(entry.get("url") or entry.get("webpage_url") or "")
            match = re.search(r"/pin/([0-9]{6,})", eurl)
            pid = match.group(1) if match else (eid if eid.isdigit() and len(eid) >= 6 else "")
            if pid and pid not in seen:
                seen.add(pid); pin_ids.append(pid)
    except Exception as e:
        print("Direct profile extraction unavailable:", e)
    ...
    print("Unique Pin IDs discovered:", len(pin_ids), "| probing:", len(urls))
    return urls
...
def _probe_pin(pin_url, cfg):
    try:
        ...
        print("Skipping non-video/unavailable pin:", pin_url, e); return None
    ...
    if not has_video: return None
    return {"url": info.get("webpage_url") or pin_url, "id": str(info.get("id") or ""), "title": (info.get("title") or info.get("description") or "Trading Short").strip(), "description": (info.get("description") or "").strip(), "duration": info.get("duration"), "timestamp": info.get("timestamp") or info.get("release_timestamp") or 0, "view_count": info.get("view_count") or 0, "like_count": info.get("like_count") or 0}
...
def discover_from_source(source_url, cfg):
    ...
    pin_urls = _get_profile_pin_urls(source_url, cfg); candidates = []
    for pin_url in pin_urls:
        item = _probe_pin(pin_url, cfg)
        if item: candidates.append(item)
    print("Usable video candidates from source:", len(candidates)); return candidates


def download_candidate(item, cfg):
    with yt_dlp.YoutubeDL(_opts(cfg, True)) as ydl:
        info = ydl.extract_info(item["url"], download=True)
        if not info: raise RuntimeError("Could not download Pinterest video.")
