...
def collect_candidates():
    ...
    # Preferred sources are checked first, but candidates must still pass every
    # duration, duplicate, policy and quality check.
    sources.sort(key=lambda s: 0 if _preferred_source(s) else 1)
    ...
    for source in sources:
        ...
        for item in items:
            ...
            item["_source_origin"] = source
            if preferred:
                item["_source_priority_bonus"] = float(CFG.get("discovery", {}).get("preferred_source_bonus", 8.0) or 0)
...
def main():
    ...
    for attempt, picked in enumerate(approved_ranked[:max_attempts], start=1):
        ...
        try:
            ...
            print("PRIVATE UPLOAD SUCCESSFUL", video_id)
            append_history(history_path, picked["_history_key"])
            append_history(hash_history_path, video_hash)
            append_metadata_history(meta, picked, video_id, CFG)
            publish_private_video_after_delay(youtube, video_id, CFG)
            return
        except Exception as exc:
